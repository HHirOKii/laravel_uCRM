...
class AnalysisService
{
    ...
    public static function perMonth($subQuery){
        $query =$subQuery->where('status', true)
            ->groupBy('id')
            ->selectRaw('id, sum(subtotal) as totalPerPurchase, customer_name, status, DATE_FORMAT(created_at, "%Y%m") as date');
    ...
    public static function perYear($subQuery){
        $query =$subQuery->where('status', true)
            ->groupBy('id')
            ->selectRaw('id, sum(subtotal) as totalPerPurchase, customer_name, status, DATE_FORMAT(created_at, "%Y") as date');
